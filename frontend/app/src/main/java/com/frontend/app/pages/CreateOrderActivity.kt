...
class CreateOrderActivity : AppCompatActivity() {
    ...
    private val orderTypeOptions = listOf("Dine In", "Take Away")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val optionOrderTypeLayout = findViewById<TextInputLayout>(R.id.optionOrderTypeLayout)
        val optionOrderType = findViewById<AutoCompleteTextView>(R.id.optionOrderType)
        ...
        recyclerProduct.layoutManager = LinearLayoutManager(this@CreateOrderActivity)
        recyclerOrderDetail.layoutManager = LinearLayoutManager(this@CreateOrderActivity)
        ...
        btnSubmit.setOnClickListener {
            ...
            if (selectedOrderType == null) {
                optionOrderTypeLayout.error = "Order type is required"
                isValid = false
            }

            if(selectedOrderType  == "Dine In" && selectedTableNumber === null){
        ...
        val orderTypeAdapterOption = ArrayAdapter(
            this@CreateOrderActivity,
            android.R.layout.simple_dropdown_item_1line,
            orderTypeOptions
        )
        optionOrderType.setAdapter(orderTypeAdapterOption)
        optionOrderType.setOnItemClickListener { _, _, position, _ ->
            val selected = orderTypeOptions[position].toString()
            selectedOrderType = selected
            if (position == 0) {
                optionTableNumberLayout.visibility = View.VISIBLE
                btnSubmit.setText("SAVE ORDER")
            } else {
                optionTableNumberLayout.visibility = View.GONE
                btnSubmit.setText("CHECKOUT NOW")
            }
        }
        ...
        bottomNav.selectedItemId = R.id.nav_order_menu

        val rootView = findViewById<View>(android.R.id.content)
        rootView.viewTreeObserver.addOnPreDrawListener(object : ViewTreeObserver.OnPreDrawListener {
            override fun onPreDraw(): Boolean {
                rootView.viewTreeObserver.removeOnPreDrawListener(this)
                loadData(this@CreateOrderActivity)
                return true
            }
        })
    ...
    private fun loadData(context: Context) {
        try {
            ...
            val request = AppHelper.getHttpRequest(context, "api/order/initial")
    ...
    private fun onSubmit() {
        try {
            ...
            val context = this@CreateOrderActivity
            ...
            val checkout = if (selectedOrderType === "Take Away") 1 else 0
            ...
            val order_type = selectedOrderType
            val status =  if (selectedOrderType === "Take Away") 1 else 0
            val cart = cart
            val table_number =  if (selectedOrderType === "Dine In") selectedTableNumber else "Take Away"
                    ...
                    (context as Activity).runOnUiThread{
                        Handler(Looper.getMainLooper()).postDelayed({
                            ...
                            if(response.code == 200){
                                ...
                                intent.putExtra("tabActive", if (selectedOrderType === "Take Away") "history" else "order")
                                startActivity(intent)
                            }else{
