...
async function inital(req, res) {

    let min = 1
    let max = 1000
    let indexes = Math.floor(Math.random() * (max - min + 1) + min)
    let orderNumberGenerate = String(indexes).padStart(5, '0')
    let dateIndex = moment(new Date()).format("YYYYMMDD")
    ...
    let cart = []
    let order = {
        order_number: `${dateIndex}${orderNumberGenerate}`,
        table_number: "",
        order_type: "Take Away",
        customer_name: `Customer ${orderNumberGenerate}`,
...
async function save(req, res) {
    ...
    cartData.forEach(async (row) => {
        ...
        let menu = await Menu.findOne({ name: row.menu_name })
        let qty = row.qty
        let price = row.price
        let total = row.total
